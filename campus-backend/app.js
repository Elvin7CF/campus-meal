...
// * 代表匹配所以请求，all代表请求可以是get、post、options
app.all('*', (req, res, next) => {
  ...
  // 强缓存，在实际开发中不合理，不可能在所有请求都进行强缓存，用户拿到的永远是老数据，这里应该根据实际情况对不同请求进行缓存
...
// 连接mongodb，创建一个session数据库，用于存储用户信息
...
app.use(session({
  name: 'mt-session',//返回客户端key的名称
  secret: 'meituan',// 服务端生成的签名
  resave: true,// 客户端并行发送多个请求，其中一个请求在另一个请求结束时覆盖
  saveUninitialized: false, // 初始化session是否保存到存储
  cookie: {
    httpOnly: true,
    secure: false,
